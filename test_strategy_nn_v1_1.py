...
path_last_month_folder = "stock_data/std_filtered/"
#filtered_vol_std
...
def add_bollinger_bands_td_df_list(df_to_scale):
    period = 20
#    multiplier = 2
    df = df_to_scale.drop('symbol', axis=1).copy()
...
def trasfroem_df_to_feed_to_trivial_cnn(df_to_transform, frame_size):
    transformed_df = add_bollinger_bands_td_df_list(df_to_transform[:frame_size])
...
#    transformed_df = np.array(transformed_df)
    transformed_df = transformed_df.reshape((1, frame_size, transformed_df.shape[1], 1))
...
def evaluate_dl_list_for_reslusts(df_list, nn_threshold, target_goal_percent,
    ...
    for i in range(len(df_rawframe_X)):
        for df in df_rawframe_X[i]:
            if(len (df) > frame_size):
                total_counts = count_success + count_failure
#                    trasfroem_df_to_feed_to_trivial_cnn
#                df_to_feed = get_single_df_to_feen_to_subsequent_nn(df)
                df_to_feed = trasfroem_df_to_feed_to_trivial_cnn(df)
                parameter_length = len(df_to_feed.columns)
                array_to_feed_subsequent_nn = np.array(df_to_feed, dtype=float).reshape(1, parameter_length, 1)
                nn_score = subsequent_model.predict(array_to_feed_subsequent_nn)
                if (nn_score > threshold):
...
#        trasfroem_df_to_feed_to_trivial_cnn
#        df_to_feed = get_single_df_to_feen_to_subsequent_nn(df)
        df_to_feed = trasfroem_df_to_feed_to_trivial_cnn(df, frame_size)
        parameter_length = df_to_feed.shape[2]
#        array_to_feed_subsequent_nn = np.array(df_to_feed, dtype=float).reshape(1, parameter_length, 1)
        nn_score = model_trivial_dnn.predict(df_to_feed)
#        nn_score = subsequent_model.predict(array_to_feed_subsequent_nn)
#        print(nn_score)
        if (nn_score > threshold):
...
def evaluate_cached_dl_list_for_reslusts(cached_indices_list, nn_threshold, nn_threshold_upper, target_goal_percent,
                                  stop_loss_percent, verbose = 1):
    count_success = 0
    count_failure = 0
    successes_value = []
    ...
    for index_in_cached_list in range(len(cached_indices_list)):
        for i,j, score in cached_indices_list[index_in_cached_list]:
            ...
            nn_score = score
            total_counts = count_success + count_failure
            if ((nn_score > threshold) and (nn_score <= nn_threshold_upper)):
                if (verbose >= 2):
...
verbose = 1
def print_result_for_specific_parameters(threshold, threshold_upper, target_goal_percent, stop_loss_percent, cached_evaluation_cropped_list):
    successes,failures, s_values, v_values, p_win, p_loss = evaluate_cached_dl_list_for_reslusts(cached_evaluation_cropped_list, threshold, threshold_upper, target_goal_percent,
...
goals = np.linspace(0.02, 0.036, 13)
...
for x in goals:
    for y in stops:
        limits.append((x,y))
for x in limits:
    print_result_for_specific_parameters(threshold, threshold_upper, x[0], x[1], cached_evaluation_cropped_list)
