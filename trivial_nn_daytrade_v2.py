...
folderlist_stock_data = get_stock_data_folderlist_from_folder(path_stock_data)[:50]
...
for i in range(int(len(folderlist_stock_data) / file_batch_size) + 1):
    ...
    df_X, df_Y = get_X_Y_data_from_folderlist(
...
#X_test = []
#Y_test = []
#date_threshold_train_test = datetime.datetime.now() - timedelta(days=90)
#for i in range(len(df_all_X)):
#    if (datetime.datetime.strptime(df_all_X[i].index[0], '%Y-%m-%d %H:%M:%S') < date_threshold_train_test):
...
#X_test = X_test.reshape(X_test.shape[0], frame_size, 10, 1)
...
history_binary2 = model_binary2.fit(X_train,np.array(Y_train),verbose=1,batch_size=20, shuffle=True,
