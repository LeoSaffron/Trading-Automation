...
time_of_trade_start_str = "2021-03-09 09:30:00"
frame_size = 100
...
def minutely_process_metronome_checker(start_time, verbose = 0):
    ...
    while (times < 99111 and continue_threads == True):
...
def get_goal_single_support_price_list_by_score_for_df(df):
    result_list = []
    for row_index in range(len(df)):
        result_row = round(float( tickers_dataframes_dict[df.iloc[row_index]['name']][-1:]['Close']) * 0.991, 2)
        result_list.append(result_row)
    return result_list
...
def send_buy_order_for_one_share(symbol, quantity, goal_price_sell, price = 0):
    stock_contract = Stock(symbol, 'SMART', 'USD')
#    c = ib.reqContractDetails(stock_contract)
    buy_order = MarketOrder('BUY', quantity)
    trade = ib.placeOrder(stock_contract, buy_order)
    while not trade.isDone():
        ib.waitOnUpdate()
    sell_order = LimitOrder('SELL', quantity, goal_price_sell)
    sleep(5)
    trade = ib.placeOrder(stock_contract, sell_order)
    while not trade.isDone():
        ib.waitOnUpdate()
    print("Placed sell order stock {} quantity {} price {}".format(symbol, quantity, goal_price_sell))
    pass


def place_order_bracket_buy(symbol, direction, qty, close_price, tp, sl):
    stock_contract = Stock(symbol, 'SMART', 'USD')
    ...
    for ord in bracket_order:
        ib.placeOrder(stock_contract, ord)
...
#            orders_sent_tws.append(place_simple_order_tws.place_order(ticker, primary_exchange, quantity, "BUY", order_type="MKT"))
        low_limit_price = tickers_dataframes_dict[ticker].iloc[frame_size - 1]['Close'] * 0.991
        low_limit_price = round(low_limit_price, 2)
        print("buying stock {}. price {}. quantity {}. support {}".format(row['name'], price, quantity, low_limit_price))
        place_order_bracket_buy(ticker, "BUY", quantity, price, row['goal_price'], low_limit_price)
#            orders_sent_tws.append(send_buy_order_for_one_share(ticker, quantity, row['goal_price']))
...
def place_buy_orders_for_dict():
    ...
    scored_df['support_price'] = get_goal_single_support_price_list_by_score_for_df(scored_df)
    for i in range(max_frames_to_buy_current):
...
while (current_test_minute < 108 and continue_threads == True):
#    global potenital_gains_tickers_first_nn
    if(len(potenital_gains_tickers_first_nn) > 0):
        try:
            ib.connect('127.0.0.1', 7497, clientId=1)
