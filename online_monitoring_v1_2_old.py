...
time_of_trade_start_str = "2020-12-11 09:30:00"
frame_size = 100
...
path_trivial_second_cnn = "measured_stats/models_results/trivial_cnn/2020_12_14__01_34_49"
...
model_trivial_second_cnn = keras.models.load_model(
    path_trivial_second_cnn +"/model.h5",
    custom_objects=None,
    compile=False
)
...
def add_bollinger_bands_to_trivial_second_cnn(df_to_scale, period, tempzeros):
    df = np.array(df_to_scale.drop('symbol', axis=1), dtype='float')
#    df = np.array(df)
    bands2 = talib.BBANDS(df.T[3], timeperiod=period, nbdevup=2, nbdevdn=2)
    bands3 = talib.BBANDS(df.T[3], timeperiod=period, nbdevup=3, nbdevdn=3)
    ...
    df = np.array([df.T[0], df.T[1], df.T[2], df.T[3], df.T[4],
                           upperBand_Width2, lowerBand_Width2, upperBand_Width3, lowerBand_Width3,
                           sma]).T
    return df

def scale_and_rearrange_df_to_trivial_second_cnn(df_to_scale, period, tempzeros):
    high = df_to_scale.T[0].max()
    low = df_to_scale.T[1].min()
    delta = high - low
    high_col = (df_to_scale.T[0] - low) / delta
    low_col = (df_to_scale.T[1] - low) / delta
    open_col = (df_to_scale.T[2] - low) / delta
    close_col = (df_to_scale.T[3] - low) / delta
    
    bollinger_band_width_2_upper = (df_to_scale.T[5] - low) / delta
    bollinger_band_width_2_lower = (df_to_scale.T[6] - low) / delta
    bollinger_band_width_3_upper = (df_to_scale.T[7] - low) / delta
    bollinger_band_width_3_lower = (df_to_scale.T[8] - low) / delta
    ...
    high_volume = df_to_scale.T[4].max()
    low_volume = df_to_scale.T[4].min()
    delta_volume = high_volume - low_volume
    volume_col = (df_to_scale.T[4] - low_volume) / delta_volume
    
    padding_col = np.zeros(df_to_scale.shape[0])
    return  np.array([padding_col, bollinger_band_width_3_upper, bollinger_band_width_2_upper, high_col, high_col, close_col, close_col, open_col, open_col,
            low_col, low_col, bollinger_band_width_2_lower, bollinger_band_width_3_lower, padding_col, volume_col, volume_col, padding_col]).T

def transform_df_to_trivial_second_cnn(df_to_transform, frame_size, period, tempzeros):
    transformed_df = (add_bollinger_bands_to_trivial_second_cnn(df_to_transform[:frame_size], period, tempzeros))
    transformed_df = scale_and_rearrange_df_to_trivial_second_cnn(transformed_df, period, tempzeros)
...
def get_single_df_to_feen_to_first_nn(df):
    df_cast_type(df)
    transformed_df = trasfroem_df_to_feed_to_trivial_cnn(df.copy(), frame_size)
    prediction_trivial_cnn = model_trivial_dnn.predict(transformed_df)[0][0]
    return prediction_trivial_cnn

def get_single_df_to_feed_to_trivial_second_nn(df):
    transformed_df = transform_df_to_trivial_second_cnn(df.copy(), frame_size, period, tempzeros)
    prediction_trivial_cnn = model_trivial_second_cnn.predict(transformed_df)[0][0]
    return prediction_trivial_cnn
