# -*- coding: utf-8 -*-
"""
Created on Sat Oct  3 16:56:25 2020
...
#pattern_count = 1050
#limit_X = 30000
threshold_Y_binary_percent = 1.015
...
new_path_stock_data = "stock_data/fetched_by_iqfeed_with_0volume" 
...
def fill_0_volume_rows_in_dflist_by_indices(dflist, ticker_index, df_index):
    ...
    for i in range(1,len(d)):
        ...
        if(time_delta != timedelta(minutes=1)):
            minutes_count = time_delta / timedelta(minutes=1) - 1
#            print(minutes_count)
            for j in range(int(minutes_count)):
                new_date = d.index[i - 1] + timedelta(minutes=1) * (j+1)
...
def get_X_Y_data_from_filelist(filelist):
    ...
    for df in clean_df:
        sym = df[0].iloc[0]['symbol']
        try:
            new_output_tickers_folder = new_path_stock_data
#            new_output_tickers_folder = fetched_by_iqfeed_with_0volume
            outdir = new_output_tickers_folder + sym
    #        print(outdir)
            if not os.path.exists(outdir):
                os.mkdir(outdir)
            new_output_tickers_folder + "{}/{}.csv".format(sym,date.today().strftime("%Y_%m_%d"))
#            filename = "%s.csv" % sym
            filename = new_output_tickers_folder + "{}/{}.csv".format(sym,date.today().strftime("%Y_%m_%d"))
            df.to_csv(filename)
        except KeyboardInterrupt:
            break
        except ValueError:
            print( "error")
            faulty_tickers.append(sym)
            print( "Error in ticker {}",sym)
        except:
            print( "Unexpected error:", sys.exc_info()[0])
            faulty_tickers.append(sym)
            print( "Error in ticker {}",sym)
...
#    return 1, 2, test_potential_volatility_list
