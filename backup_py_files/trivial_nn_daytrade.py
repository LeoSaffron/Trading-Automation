...
#                potential_volalitily = cap / vol
                ...
                if(my_is_number(potential_volalitily)):
                    ...
                    if ((potential_volalitily >= minimum_boundry) and 
                        ...
                        result_indices.append((ticker_index, day_index, vol / cap))
...
X_train, X_test, Y_train, Y_test = train_test_split(np.array(df_all_X), df_all_Y_binary, test_size=0.30)
...
#X_test = []
#Y_test = []
#date_threshold_train_test = datetime.now() - timedelta(days=14)
#for i in range(len(df_all_X)):
#    if (df_all_X[i].index[0] < date_threshold_train_test):
...
model_binary.compile( optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001),
                     loss='mse',  metrics=['accuracy', precision, recall])
