# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 01:36:00 2020
...
#        mydata = ''
        ...
        try:
            ...
            print("An exception occurred with ticker {} . function: get_stock_data_folderlist_from_folder".format(path_ticker))
            faulty_tickers.append(path_ticker)
            continue
    return df_files
...
def get_X_Y_data_from_folderlist(folder_path, frame_length):
    ...
    df_X = filter_df_list_by_days_passed(clean_df, 4)
    return df_X
...
results = std_filter_getter.get_result_ticker_std_tuples("2020-10-16", 0.01, 0.1, "stock_data/iqfeed_last_months2", 0, 0)
results = list(np.array(results).T[0])
newlist = []
for item in folderlist_stock_data:
    if item[0] in results :
        newlist.append(item)
folderlist_stock_data = newlist
del(newlist)
...
#    X_train = X_train.reshape(X_train.shape[0], parameter_length, 1)
    ...
    current_df_date = df_cropped.index[0].replace("-", "/")
...
for date in dates_to_check:
    temp_lst = get_indices_to_evaluate_by_date(df_rawframe_X, date, 200, threshold = threshold)
    print(str(datetime.datetime.now()).split('.')[0])
...
def print_result_for_specific_parameters(threshold, target_goal_percent, stop_loss_percent, cached_evaluation_cropped_list):
    ...
    if (cached_evaluation_cropped_list_total_size > 0):
        average_percent_gain = total_percent_gain / cached_evaluation_cropped_list_total_size
    else:
        average_percent_gain = 0
    print("----------------------------------------------------")
    print("threshold: {}, target %: {}, stop loss %: {}".format(
            threshold, 100 * target_goal_percent, 100 * stop_loss_percent))
    print("average percent gain {:.3f}% . wins: {}. losses: {}".format(100 * average_percent_gain,successes,failures))


goals = np.linspace(0.02, 0.026, 7)
stops = np.linspace(0.01, 0.02, 6)
limits = []
for x in goals:
    for y in stops:
        limits.append((x,y))
