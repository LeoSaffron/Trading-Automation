...
time_of_trade_start_str = "2021-01-06 09:30:00"
frame_size = 30
...
ideal_target_price = 200
ideal_target_price_max_lower_margin = 80
time_first_filter_sufficient_size = 26
volume_filter_first_threshold = 180000

shares_to_test_with_nn = []

arr_monitor_mins_to_check = [2,1]
arr_monitor_mins_to_wait_after_check = [20,20]
...
#        print(count_ticker_df_size(tickers_dataframes_dict, current_ticker))
        ...
        elif(count_ticker_df_size(tickers_dataframes_dict, current_ticker) >= time_first_filter_sufficient_size):
            current_vol = np.array(tickers_dataframes_dict[current_ticker]['Volume'][-13:], dtype=int).sum()
            if(current_vol < volume_filter_first_threshold):
                for lst in ticker_divided_list:
                    if current_ticker in lst:
                        lst.remove(current_ticker)
    for f in faulty:
...
def minutely_process_metronome_checker(start_time, verbose = 0):
    ...
    while (times < 999990 and continue_threads == True and current_test_minute < 60):
...
#        print("metronome checker {}. current time {}".format(times, newtime))
        if(is_first_time_passed_certain_minute(newtime - timedelta(minutes=(0 + 0))  -  timedelta(seconds=1), current_test_minute)):
#        if(is_first_time_passed_certain_minute(newtime, current_test_minute)):
            ...
            fetch_divided_list_of_tickers(ticker_divided_list, date_to_fetch, date_to_fetch, pre_minute_str, next_minute_str, 0.135)
...
def monitor_frames_with_sufficient_length():
    ...
    global threshold
    global continue_threads
    while (current_test_minute < 100 and continue_threads == True):
...
#            }
            ticker = shares_to_test_with_nn[0]
            df_to_check = tickers_dataframes_dict[ticker]
            close, vol, sharpe = get_score_parameters_sharpe_single_df_forsmall_value(df_to_check)
            flag_score_small = filter_single_df_by_scores_forsmall_value(close, vol, sharpe)
            flag_score_mid_small = filter_single_df_by_scores_forsmall_2_5_value(close, vol, sharpe)
            newitem = {
                    'name' : ticker,
                    'close' : close,
                    'vol' : vol,
            ...
            if((flag_score_small == True) or (flag_score_mid_small == True)):
...
def monitor_frames_with_sufficient_length_new():
    global shares_to_test_with_nn, tickers, potenital_gains_tickers, tickers_that_didnt_pass_filter
    global threshold
    global continue_threads
...
#            if(score > threshold):
#                potenital_gains_tickers_first_nn.append(newitem)
            ticker = shares_to_test_with_nn[i]
            df_to_check = tickers_dataframes_dict[ticker]
            close, vol, sharpe = get_score_parameters_sharpe_single_df_forsmall_value(df_to_check)
            flag_score_small = filter_single_df_by_scores_forsmall_value(close, vol, sharpe)
            flag_score_mid_small = filter_single_df_by_scores_forsmall_2_5_value(close, vol, sharpe)
...
def filter_single_df_forsmall_value(df_to_check):
    df = df_to_check[:100]
    ...
    last_volume = df['Volume'][85:100].sum()
    if (last_volume < 369000):
        return False
    sharpe = get_sharpe_for_sub_df_daily(df)
    if(sharpe < 0.001):
        return False
    return True

def filter_single_df_by_scores_forsmall_value(close, last_volume, sharpe):
    if (close > 2):
        return False
    if (last_volume < 169000):
        return False
    if(sharpe < 0.001):
        return False
    return True

def filter_single_df_by_scores_forsmall_2_5_value(close, last_volume, sharpe):
    if (close < 2):
        return False
    if (close > 5):
        return False
    if (last_volume < 230000):
        return False
    if(sharpe < 0.0001):
        return False
    if(sharpe < 0.001):
...
def get_score_parameters_sharpe_single_df_forsmall_value(df_to_check):
    df = df_to_check[:frame_size]
    df['Close'] = pd.to_numeric(df['Close'])
    close = df['Close'].iloc[-1]
    df['Volume'] = pd.to_numeric(df['Volume'])
    last_volume = df['Volume'][frame_size - 15 :].sum()
    sharpe = get_sharpe_for_sub_df_daily(df)
    return (close, last_volume, sharpe)


def filter_single_df_forsmall_2_5_value(df_to_check):
    df = df_to_check[:frame_size]
    df['Close'] = pd.to_numeric(df['Close'])
    close = df['Close'].iloc[-1]
    if (close < 2):
    ...
    if (close > 5):
        return False
    df['Volume'] = pd.to_numeric(df['Volume'])
    last_volume = df['Volume'][frame_size - 15 :frame_size].sum()
    if (last_volume < 369000):
        return False
    sharpe = get_sharpe_for_sub_df_daily(df)
    if(sharpe < 0.001):
...
#scores = []
#for i in range (len(shares_to_test_with_nn)):
#    ticker = shares_to_test_with_nn[i]
#    df_to_check = tickers_dataframes_dict[ticker]
#    close, vol, sharpe = get_score_parameters_sharpe_single_df_forsmall_value(df_to_check)
#    score = filter_single_df_by_scores_forsmall_value(close, vol, sharpe)
#    scores.append(score)
#print(np.where(np.array(scores) == True))
#print(shares_to_test_with_nn[88])
#
#
#scores = []
#for i in range (len(shares_to_test_with_nn)):
#    ticker = shares_to_test_with_nn[i]
#    df_to_check = tickers_dataframes_dict[ticker]
#    score = filter_single_df_forsmall_2_5_value(df_to_check)
#    scores.append(score)
#print(np.where(np.array(scores) == True))
#print(shares_to_test_with_nn[258])
...
#            orders_sent_tws.append(place_simple_order_tws.place_order(ticker, primary_exchange, quantity, "SELL", price=float("{:.2f}".format(price))))
            orders_sent_tws.append(place_simple_order_tws.place_order(ticker, primary_exchange, quantity, "SELL", order_type="MKT"))
#            global count_stocks_buy_order_placed
#            count_stocks_buy_order_placed += 1
#            global order_tickers_buy_order_sent
#            order_tickers_buy_order_sent.append(row)
...
def get_latest_price_by_name(ticker):
    ...
    for x in range((max_fetching_trials)):
        try:
            fetching_result = fetch_single_sublist_iqfeed.main([ticker], time_of_trade_start_str.split(' ')[0].replace('-',''), "093000", "160000", 0, 0)
...
def place_buy_orders_for_dict():
    ...
    scored_df = pd.DataFrame(potenital_gains_tickers_first_nn_copy).sort_values(by=['sharpe'], ascending=False)
...
#    scored_df['goal_percentage'] = get_percentage_list_by_score_for_df(scored_df)
    scored_df['goal_percentage'] =  1.05
    scored_df['goal_price'] = scored_df['close'] * scored_df['goal_percentage']
...
#tickers_path = "tickers_finnhub_all.csv"
#results_temp = pd.read_csv(tickers_path)
#results_temp = results_temp.drop('0', axis=1)
#results = []
#for x in results_temp['A']:
#    results.append((x, 1))
...
s = datetime.now()
results = filter_price_getter.get_result_ticker_close_price_tuples(time_of_trade_start_str.split(' ')[0], 2, 5, "stock_data/stocks_2019_2020", 0, 0, minimum_price = 7)
e = datetime.now()
print ( e - s )
...
for current_df_list in ticker_divided_list:
