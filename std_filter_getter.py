...
def get_tickers_within_std_volatility_bondries(df_list, minimum_boundry, maximum_boundry, minimum_price = 0):
    ...
    for df_sublist in df_list:
        ...
        try:
            if ((minimum_price > 0) and (df_sublist[0]['Close'][0] < minimum_price)):
                continue
            volatility_average = calculate_10day_average_volume_in_df_list(df_sublist, days_to_average_count = 10)
            if (volatility_average == None or volatility_average == 0):
                continue
            ticker = df_sublist[0]['symbol'][0]
            if(my_is_number(volatility_average)):
                if ((volatility_average >= minimum_boundry) and 
                    (volatility_average <= maximum_boundry)):
                    result_indices.append((ticker, volatility_average))
        except:
            pass
...
def get_ticker_tuples_data_from_filelist(filelist, lower_limit, upper_limit, verbose, error_verbose, minimum_price = 0):
    ...
    ticker_tuples = get_tickers_within_std_volatility_bondries(clean_df, lower_limit, upper_limit, minimum_price = minimum_price)
    return ticker_tuples

def get_filtered_data_tickers(start_date, end_date, path_stock_data, lower_limit, upper_limit, verbose, error_verbose, minimum_price = 0):
...
def get_result_ticker_std_tuples(date_to_check, lower_limit, upper_limit, path_stock_data, verbose, error_verbose, minimum_price = 0):
    ...
    return  get_filtered_data_tickers(start_date, end_date, path_stock_data, lower_limit, upper_limit, verbose, error_verbose, minimum_price = minimum_price)
