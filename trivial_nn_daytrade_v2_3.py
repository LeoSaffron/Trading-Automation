...
#            tf.keras.layers.Conv2D(64, (2,1), activation='relu'),
...
Y_train, Y_test, test_indices = get_train_test_Y_relative_by_threshold(0.3)
...
history_binary2 = model_binary2.fit(X_train, Y_feed,verbose=1,
                                    steps_per_epoch=300,batch_size=64, shuffle=True,
...
for x in epoch_dict[0:520]:
    t.append(x['precision_matrix_percent'][3][0.5])
plt.plot(t)
t = []
for x in epoch_dict[:1500]:
    t.append(x['total_guesses'][8:])
plt.plot(t)

model3 = get_new_model()
model3.set_weights(epoch_dict_weights[49])
...
for oi in range(len(predssss)):
    o = predssss[oi]
    if(o == True):
        if(0.3 * transform_relative_to_percentage[oi] >= 0.03):
            c += 1
            #print(oi)
print(c)
