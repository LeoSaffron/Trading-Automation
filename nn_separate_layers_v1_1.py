...
#/home/napol/cached_stocks/cached_df/201007_df_before_split
path_cached_df = "/home/napol/cached_stocks/cached_df/201110_df_before_2nd_nn/2020-11-11"
path_cached_df = "/home/napol/cached_stocks/cached_df/201110_df_before_2nd_nn"
...
#    date_in_vix = convert_date_from_df_format_to_vix_format(df_list[index_in_df].index[0])
    date_in_vix = df_list[index_in_df].index[0].split(' ')[0]
...
#                np.genfromtxt('path/to/myfile.csv',delimiter=',',skiprows=1)
                dflist.append(np.genfromtxt(folder_path + "/df_" + str(i) + ".csv", delimiter=','))
#                dflist.append(pd.read_csv(folder_path + "/df_" + str(i) + ".csv", index_col='date'))
    ...
    list_y_values = np.genfromtxt(folder_path + "_binary.csv", delimiter=',') > 0.5
    list_y_values = list(list_y_values)
    if(size > 0):
        list_y_values = list_y_values[:size]
    return dflist, list_y_values

def add_bollinger_bands_td_df_list(dflist):
    period = 20
#    multiplier = 2
    for i in range(len(dflist)):
        ...
        dflist[i] = df.fillna(0)
    return dflist

def scale_and_rearrange_df_list(df_list):
    for i in range(len(df_list)):
        df_list[i] = np.array(df_list[i])
        high = df_list[i].T[0].max()
        low = df_list[i].T[1].min()
        delta = high - low
        high_col = (df_list[i].T[0] - low) / delta
        low_col = (df_list[i].T[1] - low) / delta
        open_col = (df_list[i].T[2] - low) / delta
        close_col = (df_list[i].T[3] - low) / delta
        
        bollinger_band_width_2_upper = (df_list[i].T[5] - low) / delta
        bollinger_band_width_2_lower = (df_list[i].T[6] - low) / delta
        bollinger_band_width_3_upper = (df_list[i].T[7] - low) / delta
        bollinger_band_width_3_lower = (df_list[i].T[8] - low) / delta
        
        high_volume = df_list[i].T[4].max()
        low_volume = df_list[i].T[4].min()
        delta_volume = high_volume - low_volume
        volume_col = (df_list[i].T[4] - low_volume) / delta_volume
        
    #    df_all_X[i] = np.array([high_col, low_col, open_col, close_col, volume_col]).T
        
        padding_col = np.zeros(df_list[i].shape[0])
        df_list[i] = np.array([padding_col, bollinger_band_width_3_upper, bollinger_band_width_2_upper, high_col, high_col, close_col, close_col, open_col, open_col,
...
df_frames_to_train = []
for i in range(294290):
#for i in range(10):
    path = path_cached_df +"/df_" + str(i) + ".csv"
    df_frames_to_train.append(genfromtxt(path, delimiter=','))
...
start_process_time = datetime.datetime.now()
df_frames_to_train = df_rawframe_X.copy()
df_frames_to_train = add_bollinger_bands_td_df_list(df_frames_to_train)
end_process_time = datetime.datetime.now()
print(end_process_time - start_process_time)

start_process_time = datetime.datetime.now()
df_frames_to_train = scale_and_rearrange_df_list(df_frames_to_train)
end_process_time = datetime.datetime.now()
print(end_process_time - start_process_time)
frame_width = df_frames_to_train[0].shape[1]
#frame_width = df_cached_trivial_frame_X[0].shape[1]
...
#for i in range(len(df_frames_to_train)):
#    save_cache_path = path_cached_df + "/" + datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d") + "/df_" + str(i) + ".csv"
#    np.savetxt(save_cache_path, df_frames_to_train[i], delimiter=',')
##np.savetxt(path_cached_df + "/" + datetime.strftime(datetime.now(), "%Y-%m-%d") + ".csv", df_all_Y, delimiter=',')
#np.savetxt(path_cached_df + "/" + datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d") + "_binary.csv", df_all_Y_binary, delimiter=',')



start_process_time = datetime.datetime.now()
#trivial_nn_score = model_trivial_dnn.predict(np.array(df_cached_trivial_frame_X).reshape(len(df_cached_trivial_frame_X), frame_size, frame_width, 1))
trivial_nn_score = model_trivial_dnn.predict(np.array(df_frames_to_train).reshape(len(df_frames_to_train), frame_size, frame_width, 1))
...
X_test = []
Y_test = []
date_threshold_train_test = datetime.datetime.now() - timedelta(days=75)
for i in range(len(df_all_X)):
...
history_binary = model_binary.fit(np.asarray(X_train, dtype=float), np.array(Y_train), steps_per_epoch=200,verbose=1,batch_size=20, shuffle=True,
